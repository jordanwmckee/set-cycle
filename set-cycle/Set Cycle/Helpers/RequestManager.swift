   ...
   URLSession.shared.dataTask(with: request) { (data, response, error) in
      ...
      if (200...299).contains(httpResponse.statusCode) {
         ...
         guard let refreshToken = KeychainService.retrieveRefreshTokenFromKeychain() else {
            completion(.failure(ApiResponse(message: "unauthorized")))
            return
         }
            ...
            case .success(let data):
               do {
                  if let json = try JSONSerialization.jsonObject(with: data, options: []) as? [String: Any] {
                     ...
                     completion(.failure(ApiResponse(message: "failed to parse response")))
                  }
               } catch {
                  // JSON parsing error
                  completion(.failure(ApiResponse(message: "error: \(error)")))
               }
